...
class TestTestFileExplorer(unittest.TestCase):
    ...
    def test_parse_tag_file(self):
        tests = (os.path.join(FIXTURE_PREFIX, "one.js"),
        ...
        tags = self.test_file_explorer.parse_tag_file("js_test", os.path.join(FIXTURE_PREFIX, "tag_file1.yml"))
        # defaultdict isn't == comparable
        for test in tests:
            self.assertEqual(tags[test], expected[test])

        expected[tests[1]] = ["tag1", "tag2", "tag4"]
        tags = self.test_file_explorer.parse_tag_file("js_test", os.path.join(FIXTURE_PREFIX, "tag_file2.yml"), tags)
        for test in tests:
            self.assertEqual(tags[test], expected[test])
